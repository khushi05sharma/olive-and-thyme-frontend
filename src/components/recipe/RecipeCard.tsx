...
const RecipeCard: FC<RecipeCardProps> = ({ recipe }) => {
  ...
  // Track if user liked this recipe (local state for now)
  const [isLiked, setIsLiked] = useState<boolean>(recipe.isLiked || false);
  
  // Track if user saved this recipe
  ...
  return (
    <article
      ...
      className="overflow-hidden transition-all duration-300 bg-white shadow-md cursor-pointer group rounded-xl hover:shadow-xl"
    >
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Recipe Title */}
        <h3 className="text-lg font-bold text-gray-800 transition-colors line-clamp-1 group-hover:text-primary">
          {recipe.title}
        </h3>
